...
const htmlStr = [
                    ...
                    "        document.body.innerHTML += '<div style=\"position:fixed; top:10px; left:10px; background:white; color:red; z-index:9999; padding:20px; font-size:18px;\">ERROR: ' + msg + '<br>Line: ' + line + '<br>' + (err && err.stack ? err.stack : '') + '</div>';",
                    "    };",
                    ...
                    "            });",
                    "        }",
                    "        window.currentMapBounds = null;",
                    "        if(APP_DATA.points.length > 0) {",
                    "            let b_minLat = Infinity, b_maxLat = -Infinity, b_minLng = Infinity, b_maxLng = -Infinity;",
                    "            APP_DATA.points.forEach(p => {",
                    "                if (p.lat < b_minLat) b_minLat = p.lat;",
                    "                if (p.lat > b_maxLat) b_maxLat = p.lat;",
                    "                if (p.lng < b_minLng) b_minLng = p.lng;",
                    "                if (p.lng > b_maxLng) b_maxLng = p.lng;",
                    "            });",
                    "            window.currentMapBounds = [[b_minLat, b_minLng], [b_maxLat, b_maxLng]];",
                    "        }",
                    "        if(window.bgMapInstance && els.chkBgMap.checked && window.currentMapBounds) {",
                    "            window.bgMapInstance.invalidateSize();",
                    "            window.bgMapInstance.fitBounds(window.currentMapBounds, { animate: false });",
                    "        }",
                    ...
                    "    let bgMapLayer = null;",
                    "    els.chkBgMap.addEventListener('change', (e) => {",
                    "        const show = e.target.checked;",
                    "        const bgDiv = document.getElementById('bg-map');",
                    "        if (show) {",
                    "            bgDiv.style.display = 'block';",
                    "            if (!bgMapInstance && window.L) {",
                    "                bgMapInstance = window.L.map('bg-map', { zoomControl: false, attributionControl: false, zoomSnap: 0 });",
                    "                bgMapLayer = window.L.tileLayer('https://cyberjapandata.gsi.go.jp/xyz/std/{z}/{x}/{y}.png').addTo(bgMapInstance);",
                    "                window.bgMapInstance = bgMapInstance;",
                    "            }",
                    "            if(bgMapInstance && window.currentMapBounds) {",
                    "                bgMapInstance.invalidateSize();",
                    "                bgMapInstance.fitBounds(window.currentMapBounds, { animate: false });",
                    "            }",
                    "        } else {",
