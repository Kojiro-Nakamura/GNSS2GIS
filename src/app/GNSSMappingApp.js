...
"        <button id='btnExportHtml' class='btn-purple'>📄 HTML保存</button>",
                    ...
                    "                if (match) {",
                    "                    text_x_m = parseFloat(match[1]); ",
                    "                    text_y_m = parseFloat(match[2]); ",
                    "                }",
                    "            }",
                    "            const dpt = mToDxf(text_x_m, text_y_m);",
                    "            const hMm = (parseInt(t.fontSize) || 16) * pxToMm * S;",
                    "            dxf.addText(t.text.split(String.fromCharCode(10)).join(\' \'), dpt.x, dpt.y, hMm, 7, \'L\', typeof angle !== \'undefined\' ? angle : 0);",
                    "        });",
                    "        dxf.endGroup();",
                    "    }",
                    "",
                    ...
                    "    }",
                    "",
                    "    // Other Draggables (Tables, Boxes, Texts)",
                    "    const drawTextEl = (el) => {",
                    ...
                    "    };",
                    "",
                    "    const draggables = document.querySelectorAll(\'.print-draggable\');",
                    "    draggables.forEach(draggable => {",
                    "        if (draggable.id === \'pl-map\' || draggable.id === \'pl-compass\') return;",
                    "        ",
                    "        dxf.startGroup();",
                    "        ",
                    "        // Boxes inside draggable",
                    "        draggable.querySelectorAll(\'.pl-box\').forEach(box => {",
                    ...
                    "        });",
                    "",
                    "        // Texts inside draggable",
                    "        const textElements = Array.from(draggable.querySelectorAll(\'table td, table th, .print-title, .attr-table td, .attr-table th, .pl-table-title, .pl-summary-text\'));",
                    "        // If the draggable itself is a text element (e.g. print-title without table)",
                    "        if (draggable.classList.contains(\'print-title\')) textElements.push(draggable);",
                    "        ",
                    "        textElements.forEach(el => drawTextEl(el));",
                    "        ",
                    "        dxf.endGroup();",
                    "    });",
                    "",
                    "    // Draw remaining texts not in draggables",
                    "    const allTextElements = Array.from(document.querySelectorAll(\'#canvasWrap table td, #canvasWrap table th, .print-title, .attr-table td, .attr-table th, .pl-table-title, .pl-summary-text\'));",
                    "    const remaining = allTextElements.filter(el => !el.closest(\'.print-draggable\'));",
                    "    if (remaining.length > 0) {",
                    "        dxf.startGroup();",
                    "        remaining.forEach(el => drawTextEl(el));",
                    "        dxf.endGroup();",
                    "    }",
                    "",
