...
export class GNSSMappingApp {
            constructor() {
                ...
                this.stateManager = new StateHistoryManager(this);
                this.state = {
                    points: [],
                    edges: [],
                    areas: [],
            ...
            saveState() { this.stateManager.saveState(); }
            pushToHistory(data) { this.stateManager.pushToHistory(data); }
            undo() { this.stateManager.undo(); }
            redo() { this.stateManager.redo(); }
            restoreSnapshot(snapshot) { this.stateManager.restoreSnapshot(snapshot); }
            updateUndoRedoButtons() { this.stateManager.updateUndoRedoButtons(); }
            loadState() { this.stateManager.loadState(); }
            ...
            renderAttrTable() { this.tableRenderer.renderAttrTable(); }
            renderTable() { this.tableRenderer.renderTable(); }
            renderAreaResults() { this.tableRenderer.renderAreaResults(); }
            copyAreaToClipboard() { this.tableRenderer.copyAreaToClipboard(); }


            executeSaveJson() { this.exportService.executeSaveJson(); }
            traverseFileTree(item, files) { return this.exportService.traverseFileTree(item, files); }
            showLoadPreviewModal() { this.exportService.showLoadPreviewModal(); }
            selectPreviewFile(index) { this.exportService.selectPreviewFile(index); }
            generatePreviewSVG(points, edges, areas, data = null) { return this.exportService.generatePreviewSVG(points, edges, areas, data); }
            applyLoadData() { this.exportService.applyLoadData(); }
            executeExportGeoJSON() { this.exportService.executeExportGeoJSON(); }
            openPrintLayoutWindow() { this.exportService.openPrintLayoutWindow(); }
