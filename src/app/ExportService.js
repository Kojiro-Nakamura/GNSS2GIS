...
const htmlStr = [
        ...
        "        <select id='dxfExportUnit' style='padding:4px; font-size:12px; margin-right:5px; border-radius:4px;'><option value='paper_mm' selected>図面サイズ(ミリメートル) - 推奨</option><option value='mm'>実寸座標(ミリメートル) - CAD用</option><option value='m'>実寸座標(メートル) - GIS用</option></select><button id='btnExportDxf' style='background: #c2410c;'>💾 DXF保存</button>",
                    ...
                    "        btnClose: document.getElementById('btnClosePrint'),",
                    ...
                    "        if (lat < 30) {",
                    "            if (lng > 150) autoZoneIndex = 19; else if (lng > 140) autoZoneIndex = 14; else if (lng > 133) autoZoneIndex = 18; else if (lng > 129) autoZoneIndex = 17; else if (lng > 126) autoZoneIndex = 15; else autoZoneIndex = 16;",
                    "        } else if (lat < 33) {",
                    "            if (lng > 133) autoZoneIndex = 14; else if (lng > 131) autoZoneIndex = 3; else if (lng > 129) autoZoneIndex = 2; else autoZoneIndex = 1;",
                    "        } else if (lat < 36) {",
                    "            if (lng > 139) autoZoneIndex = 9; else if (lng > 137.5) autoZoneIndex = 8; else if (lng > 136) autoZoneIndex = 7; else if (lng > 134) autoZoneIndex = 6; else if (lng > 132.5) autoZoneIndex = 5; else autoZoneIndex = 4;",
                    "        } else if (lat < 40) {",
                    "            if (lng > 140.5) autoZoneIndex = 10; else if (lng > 139.5) autoZoneIndex = 9; else if (lng > 138.5) autoZoneIndex = 8; else autoZoneIndex = 7;",
                    "        } else if (lat < 44) {",
                    "            if (lng > 142.5) autoZoneIndex = 13; else if (lng > 141) autoZoneIndex = 12; else if (lng > 139.5) autoZoneIndex = 11; else autoZoneIndex = 10;",
                    "        } else {",
                    "            if (lng > 142.5) autoZoneIndex = 13; else if (lng > 141) autoZoneIndex = 12; else autoZoneIndex = 11;",
                    "        }",
                    "        const zoneNames = [",
                    "            '', '第1系 (長崎・佐賀など)', '第2系 (福岡・大分など)', '第3系 (山口・島根など)', '第4系 (香川・愛媛など)',",
                    "            '第5系 (兵庫・鳥取など)', '第6系 (京都・大阪など)', '第7系 (石川・富山など)', '第8系 (新潟・長野など)',",
                    "            '第9系 (東京・神奈川など)', '第10系 (青森・秋田など)', '第11系 (北海道西)', '第12系 (北海道中)', '第13系 (北海道東)',",
                    "            '第14系 (諸島南)', '第15系 (沖縄本島など)', '第16系 (石垣島など)', '第17系 (宮古島など)', '第18系 (小笠原など)', '第19系 (南鳥島など)'",
                    "        ];",
                    "        const optAuto = document.getElementById('optAutoZone');",
                    "        if(optAuto) optAuto.innerText = '平面直角座標 (' + zoneNames[autoZoneIndex] + ')';",
                    "",
                    ...
                    "        }",
                    "        addText(text, alignX, y, height, color=256, align='L', angle=0) {",
                    "            const safeText = String(text).replace(/[\\n\\r]/g, ' ');",
                    "            let baseX = alignX;",
                    "            if (align === 'C' || align === 'R') {",
                    "                let tw = 0;",
                    "                for(let i=0; i<safeText.length; i++) { tw += (safeText.charCodeAt(i) > 255) ? height : (height * 0.55); }",
                    "                if (align === 'C') baseX = alignX - tw / 2; else baseX = alignX - tw;",
                    "            }",
                    "            this.currentSection.push('0', 'TEXT', '8', '0', '62', color, '10', baseX.toFixed(4), '20', y.toFixed(4), '40', height.toFixed(4), '1', safeText);",
                    "            if (angle !== 0) this.currentSection.push('50', angle.toFixed(4));",
                    "            if (align === 'C') {",
                    "                this.currentSection.push('72', '1', '73', '0', '11', alignX.toFixed(4), '21', y.toFixed(4));",
                    "            } else if (align === 'R') {",
                    "                this.currentSection.push('72', '2', '73', '0', '11', alignX.toFixed(4), '21', y.toFixed(4));",
                    "            }",
                    "        }",
                    ...
                    "        const updateMin = (lat, lng) => { if (lat < minLat) minLat = lat; if (lng < minLng) minLng = lng; };",
                    "        if (APP_DATA.points) APP_DATA.points.forEach(p => updateMin(p.lat, p.lng));",
                    "        if (APP_DATA.customLines) APP_DATA.customLines.forEach(l => l.latlngs.forEach(c => updateMin(c[0] !== undefined ? c[0] : c.lat, c[1] !== undefined ? c[1] : c.lng)));",
                    "        if (APP_DATA.areas) APP_DATA.areas.forEach(a => a.coords.forEach(c => updateMin(c[0], c[1])));",
                    "        let p0 = {lat: 35, lng: 139};",
                    "        if (minLat !== Infinity) p0 = {lat: minLat, lng: minLng};",
                    "        let zoneIndex = 0;",
                    "        if (zoneVal === 'auto') {",
                    "            const lat = p0.lat, lng = p0.lng;",
                    "            if (lat < 30) {",
                    "                if (lng > 150) zoneIndex = 19; else if (lng > 140) zoneIndex = 14; else if (lng > 133) zoneIndex = 18; else if (lng > 129) zoneIndex = 17; else if (lng > 126) zoneIndex = 15; else zoneIndex = 16;",
                    "            } else if (lat < 33) {",
                    "                if (lng > 133) zoneIndex = 14; else if (lng > 131) zoneIndex = 3; else if (lng > 129) zoneIndex = 2; else zoneIndex = 1;",
                    "            } else if (lat < 36) {",
                    "                if (lng > 139) zoneIndex = 9; else if (lng > 137.5) zoneIndex = 8; else if (lng > 136) zoneIndex = 7; else if (lng > 134) zoneIndex = 6; else if (lng > 132.5) zoneIndex = 5; else zoneIndex = 4;",
                    "            } else if (lat < 40) {",
                    "                if (lng > 140.5) zoneIndex = 10; else if (lng > 139.5) zoneIndex = 9; else if (lng > 138.5) zoneIndex = 8; else zoneIndex = 7;",
                    "            } else if (lat < 44) {",
                    "                if (lng > 142.5) zoneIndex = 13; else if (lng > 141) zoneIndex = 12; else if (lng > 139.5) zoneIndex = 11; else zoneIndex = 10;",
                    "            } else {",
                    "                if (lng > 142.5) zoneIndex = 13; else if (lng > 141) zoneIndex = 12; else zoneIndex = 11;",
                    "            }",
                    "        } else {",
...
"            if(area.center) {",
                    ...
                    "                const boxTop = ac.y + th * 2.2;",
                    "                const boxBottom = ac.y - th * 0.7;",
                    "                const boxHalfW = (Math.max(t1.length * 1.5, t2.length * 0.8) * th) / 2 + th * 0.5;",
                    "                const boxLeft = ac.x - boxHalfW;",
                    "                const boxRight = ac.x + boxHalfW;",
                    "                dxf.addPolyline([{x: boxLeft, y: boxTop}, {x: boxRight, y: boxTop}, {x: boxRight, y: boxBottom}, {x: boxLeft, y: boxBottom}], true, 3);",
                    "            }",
"            if (area.holes) { area.holes.forEach((hole, j) => {",
                    ...
                    "                    const ht1 = '(内 除地' + (j+1) + ')';",
                    "                    const ht2 = (Math.round(((hole.area / 10000) + Number.EPSILON) * 10000) / 10000).toFixed(4) + ' ha';",
                    "                    const th = 2.5 * textScaleFactor;",
                    "                    dxf.addText(ht1, hc.x, hc.y + th*1.0, th, 1, 'C');",
                    "                    dxf.addText(ht2, hc.x, hc.y - th*0.5, th, 1, 'C');",
                    "                    const boxTop = hc.y + th * 2.2;",
                    "                    const boxBottom = hc.y - th * 0.7;",
                    "                    const boxHalfW = (Math.max(ht1.length * 1.2, ht2.length * 0.8) * th) / 2 + th * 0.5;",
                    "                    const boxLeft = hc.x - boxHalfW;",
                    "                    const boxRight = hc.x + boxHalfW;",
                    "                    dxf.addPolyline([{x: boxLeft, y: boxTop}, {x: boxRight, y: boxTop}, {x: boxRight, y: boxBottom}, {x: boxLeft, y: boxBottom}], true, 1);",
                    "                }",
                    "            }); }",
                    "        }); }",
                    ...
                    "        if (canvasWrapEl) {",
"            const oldTransition = canvasWrapEl.style.transition;",
                    "            const oldTransform = canvasWrapEl.style.transform;",
                    "            canvasWrapEl.style.transition = 'none';",
                    "            canvasWrapEl.style.transform = 'none';",
                    "            void canvasWrapEl.offsetHeight;",
                    "            const canvasRect = canvasWrapEl.getBoundingClientRect();",
                    "            ",
                    "            // Draw Paper Boundary (color 8 = dark grey)",
                    "            const pTopLeft = pxToDxfMap(0, 0);",
                    "            const pTopRight = pxToDxfMap(paperW_px, 0);",
                    "            const pBottomRight = pxToDxfMap(paperW_px, paperH_px);",
                    "            const pBottomLeft = pxToDxfMap(0, paperH_px);",
                    "            dxf.addPolyline([pTopLeft, pTopRight, pBottomRight, pBottomLeft], true, 8);",
                    "            ",
                    "            const drawHtmlElement = (el, color, drawBox = true) => {",
                    "                const rect = el.getBoundingClientRect();",
                    "                const pxLeft = rect.left - canvasRect.left;",
                    "                const pxTop = rect.top - canvasRect.top;",
                    "                const pxRight = rect.right - canvasRect.left;",
                    "                const pxBottom = rect.bottom - canvasRect.top;",
                    "                ",
                    "                if (drawBox) {",
                    "                    const p1 = pxToDxfMap(pxLeft, pxTop);",
                    "                    const p2 = pxToDxfMap(pxRight, pxTop);",
                    "                    const p3 = pxToDxfMap(pxRight, pxBottom);",
                    "                    const p4 = pxToDxfMap(pxLeft, pxBottom);",
                    "                    dxf.addPolyline([p1, p2, p3, p4], true, color);",
                    "                }",
                    "                ",
                    ...
                    "                        const textAlign = el.style.textAlign || compStyle.textAlign; let textX = tp.x; let alignFlag = 'C'; if (textAlign === 'left' || textAlign === 'start') { textX = pxToDxfMap(pxLeft + (parseFloat(compStyle.paddingLeft) || 0), textPxY).x; alignFlag = 'L'; } else if (textAlign === 'right' || textAlign === 'end') { textX = pxToDxfMap(pxRight - (parseFloat(compStyle.paddingRight) || 0), textPxY).x; alignFlag = 'R'; }",
                    "                        dxf.addText(text, textX, tp.y - fontSizeMeters * dxfScale * 0.35, fontSizeMeters * dxfScale, color, alignFlag);",
                    "                    }",
                    "                }",
                    "            };",
                    "",
            "            ['pl-attr', 'pl-area', 'pl-survey'].forEach(id => {",
            "                const elWrap = document.getElementById(id);",
            "                if (elWrap && elWrap.style.display !== 'none') {",
            "                    elWrap.querySelectorAll('.pl-table-title').forEach(el => drawHtmlElement(el, 7));",
            "                    elWrap.querySelectorAll('th, td').forEach(el => drawHtmlElement(el, 7));",
            "                }",
            "            });",
            "            const summaryWrap = document.getElementById('pl-summary');",
            "            if (summaryWrap && summaryWrap.style.display !== 'none') {",
                    "                const inner = summaryWrap.querySelector('.pl-summary-text'); if (inner) drawHtmlElement(inner, 7, false);",
            "            }",
                    "            ",
                    ...
                    "                const pxBottom = rect.bottom - canvasRect.top;",
                    "                ",
                    "                const p1 = pxToDxfMap(pxLeft, pxBottom);",
                    "                const p2 = pxToDxfMap(pxRight, pxBottom);",
                    "                const pMidTop = pxToDxfMap((pxLeft + pxRight)/2, pxTop + 24);",
                    "                const pMidBottom = pxToDxfMap((pxLeft + pxRight)/2, pxBottom - 15);",
                    "                ",
                    "                dxf.addLine(p1.x, p1.y, pMidTop.x, pMidTop.y, 7);",
                    "                dxf.addLine(p2.x, p2.y, pMidTop.x, pMidTop.y, 7);",
                    "                dxf.addLine(p1.x, p1.y, pMidBottom.x, pMidBottom.y, 7);",
                    ...
                    "            }",
                    "            ",
"            canvasWrapEl.style.transform = oldTransform;",
                    "            void canvasWrapEl.offsetHeight;",
                    "            canvasWrapEl.style.transition = oldTransition;",
                    "        }",
                    "",
